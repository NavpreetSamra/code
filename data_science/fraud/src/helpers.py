...
def select_transform_features(design, target,
    ...
    :return: transformed data and transformer
    :rtype: tuple(`pandas.DataFrame`, `sklearn.Pipeline`)
    """
...
def build_param_grid():
    ...
    g = {'lr': {'clf': LogisticRegression(class_weight='balanced'),
                'grid': {'C': np.logspace(-2, 4, 8), 'penalty': ['l1', 'l2']}},
         'rf': {'clf': RandomForestClassifier(n_estimators=200, n_jobs=-1),
                'grid': {'max_depth': [5, 10, 15],
                         'min_samples_split': [100, 150, 200]}
                },
         'svc': {'clf': SVC(kernel='rbf'),
                 'grid': {'C': [10, 100], 'gamma': [0.001, 0.0001]}
                 }
...
def stacked_bar(df, col, target, fName, rot=0, kind='bar'):
    """
    ...
    df2.plot(kind=kind, stacked=True, rot=rot)
    plt.tight_layout()
    plt.savefig(fName)
    plt.close()
