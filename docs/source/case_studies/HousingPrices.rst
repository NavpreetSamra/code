...
Built in python 2.7 with the following dependencies 
...
The contents of this package are distributed across three folders 
    
    * `src` containing source code pickled cleaner and regression objects as well as 
        
        #. :py:class:`windfall.src.preprocessor`
        #. :py:class:`windfall.src.helpers`

    * `doc` containg source and build data for documentation
    * `data` containing data used produce this analysis


To generate estimates for house values run housing_predictor.py passing the location of the file to predict on as the first argument.
The current directory ``./`` is the default output folder and accesible through the ``-d`` or ``--output_folder`` option.

housing_predictor.py will generate a new csv in the specified directory prepended by ``prediction`` and an appended prediction field with the estimated values

Examples

.. code-block:: bash

    $ python {$PACKAGEDIR}/src/housing_predictor.py {$DATA}/data.csv

    $ python {$PACKAGEDIR}/src/housing_predictor.py {$DATA}/data.csv -d {$OUTPUTFOLDER}

    $ python ~/test_env/src/housing_predictor.py my_data.csv --output_folder other_place/to_hold_data


Note
----

housing_predictor.py has local dependies, please run access from src folder.
